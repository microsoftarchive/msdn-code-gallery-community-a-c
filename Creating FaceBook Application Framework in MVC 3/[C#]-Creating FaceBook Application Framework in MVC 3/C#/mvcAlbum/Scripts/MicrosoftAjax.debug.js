...
function Sys$Enum$parse(value, ignoreCase) {
    ...
    if (ignoreCase) {
        values = this.__lowerCasmvcues;
        if (!values) {
            this.__lowerCasmvcues = values = {};
...
Sys.Net._WebRequestManager = function Sys$Net$_WebRequestManager() {
    ...
    function Sys$Net$_WebRequestManager$executeRequest(webRequest) {
        ...
        if (!executor) {
            var failed = false;
            try {
                var executorType = mvc(this._defaultExecutorType);
...
'nullReferenceInPath':'Null reference while mvcuating data path: \'{0}\'.',
