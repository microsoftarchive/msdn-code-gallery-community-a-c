...
Namespace EnumerateVolume
	Public Class PInvokeWin32
		...
		Private _directories As New Dictionary(Of ULong, FileNameAndFrn)()

		Public Property Directories() As Dictionary(Of ULong, FileNameAndFrn)
			Get
				Return _directories
			End Get
			Set(ByVal value As Dictionary(Of ULong, FileNameAndFrn))
				_directories = value
			End Set
		End Property
		...
		Public Sub EnumerateVolume(<System.Runtime.InteropServices.Out()> ByRef files As Dictionary(Of UInt64, FileNameAndFrn), ByVal fileExtensions() As String)
			files = New Dictionary(Of ULong, FileNameAndFrn)()
		...
		Private Sub GetRootFrnEntry()
			...
			If hRoot.ToInt32() <> PInvokeWin32.INVALID_HANDLE_VALUE Then
				...
				If bRtn Then
					...
					Dim f As New FileNameAndFrn(driveRoot, 0)
...
'ORIGINAL LINE: unsafe public void EnumerateFiles(IntPtr medBuffer, ref Dictionary(Of ulong, FileNameAndFrn) files, string[] fileExtensions)
		Public Sub EnumerateFiles(ByVal medBuffer As IntPtr, ByRef files As Dictionary(Of ULong, FileNameAndFrn), ByVal fileExtensions() As String)
				...
				Do While outBytesReturned > 60
					...
					If 0 <> (usn.FileAttributes And PInvokeWin32.FILE_ATTRIBUTE_DIRECTORY) Then
						...
						If Not _directories.ContainsKey(usn.FileReferenceNumber) Then
							_directories.Add(usn.FileReferenceNumber, New FileNameAndFrn(usn.FileName, usn.ParentFileReferenceNumber))
						Else
					...
					Else
						...
						If add Then
							If Not files.ContainsKey(usn.FileReferenceNumber) Then
								files.Add(usn.FileReferenceNumber, New FileNameAndFrn(usn.FileName, usn.ParentFileReferenceNumber))
							Else
								Dim frn As FileNameAndFrn = files(usn.FileReferenceNumber)
